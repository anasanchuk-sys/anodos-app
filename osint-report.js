(function(scope){
  ...
  function buildDefinition(report){
    if(report?.version!=='anodos-osint-pilot-v1'||!Array.isArray(report.findings)||!report.findings.length)throw new Error('Немає перевіреного результату для PDF.');
    ...
    if(report.rejectedClaims)content.push({text:`Не включено тверджень без перевіреної цитати: ${report.rejectedClaims}.`,style:'small',margin:[0,12,0,0]});
