...
export function qualityReportSummary(result){
 if(result.blocked)return result.summary;
 if(result.issues?.length)return 'У звіті виділено умови, які варто покращити, та виявлені неточності. Для кожного зауваження пояснюємо можливий вплив на страхове покриття або виплату.';
 if(result.unknown?.length)return 'За підтвердженими умовами конкретних зауважень не виявлено. Окремі питання потребують уточнення за документами.';
...
export function buildQualityPdf(result,{britmark,anodos}){
 const content=[{image:'britmark',width:138,margin:[0,0,0,20]},
  ...
  {text:clean(qualityReportSummary(result)),fontSize:11,lineHeight:1.3,margin:[0,0,0,15]},
  ...
  {text:'Висновок сформовано автоматично за завантаженими документами. Він може містити пропуски. Звірте зауваження з оригіналом договору. Питання, для яких бракує підтвердження, наведені окремо.',fontSize:8,color:muted,lineHeight:1.2,margin:[0,0,0,14]}];
 function section(title,checks){if(!checks?.length||result.blocked)return;checks.forEach((c,i)=>{
  const blocks=[{text:(i+1)+'. '+clean(c.title),fontSize:12,bold:true,color:ink,margin:[0,0,0,4]},
   {text:clean(c.assessment),lineHeight:1.18,margin:[0,0,0,4]}];
  if(c.impact)blocks.push({text:clean(c.impact),lineHeight:1.18,margin:[0,0,0,4]});
  ...
  content.push({stack:blocks,margin:[0,0,0,7],id:'check-'+c.id});
 });}
 section('Зауваження до договору',result.issues);if(result.unknown?.length&&!result.blocked){result.unknown.forEach((c,i)=>content.push({stack:[{unbreakable:true,stack:[...(i===0?[{text:'Що потрібно уточнити',fontSize:14,bold:true,margin:[0,14,0,8]}]:[]),{text:clean(c.title),bold:true},{text:clean(c.assessment),margin:[0,3,0,0]}]}],fontSize:9,lineHeight:1.2,margin:[0,0,0,7],id:'check-'+c.id}));}
