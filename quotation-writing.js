(function (scope) {
  ...
  function render(){
    ...
    host.innerHTML=`<section class="quotation-workspace" data-quotation-root aria-labelledby="quotationWritingTitle"><header class="contract-review-head"><button class="module-back" type="button" data-route="home" aria-label="Назад до страхування">←</button><div><h1 id="quotationWritingTitle">Написання котирувань</h1></div>${access?'<button class="secondary-action quotation-lock" type="button" data-quotation-lock>Заблокувати</button>':''}</header>
      ${!access?`<section class="quotation-gate"><h2 class="anodos-pro-access-title"><span class="anodos-pro-access-copy">введіть пароль</span> <span class="anodos-pro-access-brand">Anodos Pro</span></h2><form data-quotation-access><label for="quotationPassword">Пароль</label><input id="quotationPassword" type="password" autocomplete="current-password" required maxlength="256" ${busy?'disabled':''}/><button class="primary-action" type="submit" ${busy?'disabled':''}>Відкрити інструмент</button></form></section>`:
