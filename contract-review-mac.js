(function(scope){
  ...
  const MAX_WAIT_MS=15*60*1000,SOFT_TARGET_MS=60000;
  async function analyze(records,{read,progress,budgetMs=MAX_WAIT_MS}){
    ...
    const started=performance.now(),budget=Math.max(1,Math.min(MAX_WAIT_MS,budgetMs)),deadline=new AbortController();
    const timeout=()=>Object.assign(new Error('Перевірка перевищила захисний час очікування сервісу. Оригінали залишились в архіві Anodos; зверніться до оператора для продовження.'),{code:'review_deadline'});
    const timer=setTimeout(()=>deadline.abort(timeout()),budget);
    let currentMessage='Перевірка триває.';
    const report=message=>{currentMessage=message;progress(message+(performance.now()-started>=SOFT_TARGET_MS?' Потрібно більше часу. Перевірка продовжується, залиште вкладку відкритою.':''));};
    const softTimer=setTimeout(()=>report(currentMessage),SOFT_TARGET_MS);
    ...
    try{
      ...
      report('Підключаю локальну модель Anodos через зашифрований канал…');
      ...
      for(let index=0;index<records.length;index++){
        ...
        for(let offset=0;offset<record.file.size;offset+=opened.chunkSize){
          report(`Зберігаю оригінал ${index+1} з ${records.length}: ${Math.floor(offset/record.file.size*100)}%`);
      ...
      for(const record of records){
        report('Оригінали збережено. Читаю: '+record.name);record.requireCompleteReading=true;
      ...
      let payload;
      while(remaining()>0){
        const state=await within(call({op:'status'}));report(state.progress?.message||'Перевіряю договір на сервері Anodos…');
      ...
      if(!result.blocked){report('Результат збережено. Формую й зберігаю PDF BRITMARK…');try{await within(pdf());}catch(e){check();result.reviewWarnings.push('PDF ще не збережено в архіві. Натисніть «Завантажити PDF», щоб повторити; оригінали та результат вже збережені.');}}
      check();result.elapsedMs=Math.round(performance.now()-started);completed=true;
      return session;
    }catch(e){if(capability)void call({op:'cancel'},{cleanup:true}).catch(()=>{});throw e;}finally{clearTimeout(timer);clearTimeout(softTimer);}
  }
